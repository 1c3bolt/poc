...
*/
const mapping = require("/custom-modules/custom/newenv2/mapping.xqy");
...
function main(content, options) {
  ...
  let data = datahub.flow.flowUtils.getInstance(doc) || {};

  // get triples, return null if empty or cannot be found
  let triples = datahub.flow.flowUtils.getTriples(doc) || [];

  //gets headers, return null if cannot be found
  let headers =  mapping.mapHeader(data);
  ...
  let instance = mapping.mapDoc(data);
